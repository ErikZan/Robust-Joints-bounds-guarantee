...
while ( L != []):
    ...
    All_L.append(new_area1)
    All_L.append(new_area3)
    while ( L1 !=[]):
        ...
        while( L3 !=[]):
            ...
            if (R != []):
                L_part= R_analyzer(R,L3[0])
            classic=False    
            if (L_part ==[]):
                classic=True
            print(L_part)    
            (q,dq)=Minimize_area_viab_extend(L3[0],L_part,acc,classic=classic)
        ...
        if (R != []):
            L_part= R_analyzer(R,L1[0])
        classic=False
        if (L_part ==[]):
            classic=True
        print(L_part)
        (q,dq)=Minimize_area_viab_extend(L1[0],L_part,acc,classic=classic)
        #(new_area1,new_area2,new_area3) = new_areas(L[0],q,dq)
